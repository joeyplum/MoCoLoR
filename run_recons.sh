...
run_binning_quantile() {
    $PYTHON_EXECUTABLE binning_hilbert_dynamic.py --fname data/floret-740H-035c/ --nbins 10 --plot 1 --nprojections 10000
}

# Function to run the recon_xdgrasp_npy.py script with given arguments
run_recon_xdgrasp1() {
    $PYTHON_EXECUTABLE recon_xdgrasp_npy.py data/floret-740H-035c/ --lambda_TV 0.05 --vent_flag 1 --recon_res 117 --scan_res 220
}

# Function to run the recon_mocolor_npy.py script with given arguments
run_recon_mocolor1() {
    $PYTHON_EXECUTABLE recon_lrmoco_vent_npy.py data/floret-740H-035c/ --lambda_lr 0.05 --vent_flag 1 --recon_res 117 --scan_res 220 --mr_cflag 1 --iner_iter 50 --res_scale 0.7
}

# Function to run the recon_xdgrasp_npy.py script with given arguments
run_recon_nufft() {
    $PYTHON_EXECUTABLE recon_dcf_nufft_npy.py data/floret-740H-035c/ --lambda_TV 0 --vent_flag 0 --recon_res 117 --scan_res 220
}

# Generate a mask
run_segmentation() {
    python segmentation/segmentation_ute.py --fname data/floret-740H-035c/results/ --filename img_mocolor_20_bin_117_resolution.nii --plot 0 --mask 1
}
